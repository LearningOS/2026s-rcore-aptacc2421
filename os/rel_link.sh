...
# 相对路径（从 os/ 目录出发）
USER_ELF_DIR="../user/target/riscv64gc-unknown-none-elf/release"

# 收集所有 ch* 文件
apps=()
for f in /mnt/user/target/riscv64gc-unknown-none-elf/release/ch*; do
    if [ -f "$f" ] && [ ! -x "$f" ] || [ -x "$f" ]; then
        base=$(basename "$f")
...
echo "生成 $OUTPUT，包含 ${#apps[@]} 个应用（使用相对路径）"
